	...
	\begin{enumerate}[nolistsep]
		\item Design a discrete-time second-order Butterworth lowpass filter with cutoff frequency equivalent to $f_c = 1.6 \text{kHz}$. Assume the ADC sampling frequency is $f_s = 8 \text{kHz}$. Use the impulse invariance method with $T_d = 1$.
			...
			\begin{enumerate}[nolistsep]
				...
				We first need to find $h_a(t)$ by taking an inverse Laplace transform of $H_a(s)$. We must first use partial fraction expansion to write $H_a(s)$ in the following form:
				...
				\begin{equation*}
					h_a(t) = \frac{-j\pi\sqrt{2}}{5}e^{0.4}
				\end{equation*}
			\end{enumerate}
	\end{enumerate}
\end{document}
